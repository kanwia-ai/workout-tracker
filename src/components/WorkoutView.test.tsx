...
describe('WorkoutView manual rest timer', () => {
  it('opens the TimerOverlay when the clock button is tapped on a row', () => {
    vi.mocked(usePlan).mockReturnValue({ plan: makePlan([makeSession()]), loading: false })
    render(<WorkoutView {...baseProps} />)
    // Each row has an "Open rest timer for {name}" button.
    const restBtn = screen.getByRole('button', {
      name: /Open rest timer for romanian deadlift/i,
    })
