...
8.6 **Cardio placement — judgment from these principles** (per docs/research/02-coaching-philosophy.md §"Don't confuse the body"):
       The body has a state. Heavy cardio before lifting puts the body in cardiovascular mode and degrades strength output for the rest of the session. So:
         ...
         • If the user's primary_goal is fat loss / cutting / body-composition → this is a workout plan and the goal is fat loss, so DO prescribe cardio as a standard part of the program (cardio_policy is 'separated'): place it AFTER strength (HR is already elevated) or on separate days. Diet still drives the actual fat loss and the lifting protects muscle, but cardio is the training-side lever for the deficit — include it by default, don't bury it as an opt-in.
